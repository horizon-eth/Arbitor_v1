...
const poolABI = require(path.join(__dirname, `../../../Scripts/Dex/PoolABIsV2/ClipperV2PoolABI.json`));
...
let POL_USDC_POOL = {
	...
	https_contract: new ethers.Contract("0xAe00af61bE6861eE956C8e56BF22144d024acb57", poolABI, POOL_PROVIDER_V2),
	wss_contract: null,
	reserve0: undefined,
	reserve1: undefined,
	processing: false,
...
// 0x37eAA0eF3549a5Bb7D431be78a3D99BD360d19e5 USDC
...
async function Executor(pool) {
 ...
 -- entranceProfit = ${entranceProfit}$`
	...
	const selector = await flashSwapContract.getSelectorID("swap(address,address,uint256,uint256,uint256,address,(uint8,bytes32,bytes32),bytes)");
	// const selector = await flashSwapContract.getSelectorID("swap(address,address,uint256,uint256,uint256,address,(uint8,bytes32,bytes32),bytes)");
	console.log(selector);

	return;

	const tx_data = ethers;

	const tx2 = {
		from: WalletAddress,
		to: POL_USDC_POOL.address,
		data: tx_data,
		chainId: chainID,
		value: 0,
		type: 0,
	...
	async function simulate() {
		...
		try {
			await FLASHSWAP_PROVIDER.call(tx2);
...
async function Initializer() {
	for (const pool of poolsData) {
		...
		const [reserves, [tokens]] = await pool_object.https_contract.allTokensBalance();
...
async function Listen() {
	...
	async function addListeners() {
		for (const pool of poolsData) {
			...
			console.log("Old Listener Count of", pool_object.address, pool_object.wss_contract.listenerCount("Swapped"));

			await pool_object.wss_contract.removeAllListeners("Swapped");

			console.log("New Listener Count of", pool_object.address, pool_object.wss_contract.listenerCount("Swapped"));

			pool_object.wss_contract.on("Swapped", async (inAsset, outAsset, recipient, inAmount, outAmount, auxiliaryData) => {
				try {
					console.log(`Before if else reserve0: ${pool_object.reserve0} reserve1: ${pool_object.reserve1}`);

					if (inAsset == pool.token0_address) {
						pool_object.reserve0 += BigInt(inAmount);
						pool_object.reserve1 -= BigInt(outAmount);
					} else {
						pool_object.reserve0 -= BigInt(outAmount);
						pool_object.reserve1 += BigInt(inAmount);
					}

					console.log(`inAmount: ${inAmount} // outAmount: ${outAmount}`);

					console.log(`After if else reserve0: ${pool_object.reserve0} reserve1: ${pool_object.reserve1}`);
			...
			console.log("Added Listener Count of", pool_object.address, pool_object.wss_contract.listenerCount("Swapped"));
		}
	}
