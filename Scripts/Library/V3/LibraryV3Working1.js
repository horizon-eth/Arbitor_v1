		...
		if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
			...
			if (step.initialized) {
				...
				if (!cache.computedLatestObservation) {
					...
					[cache.tickCumulative, cache.secondsPerLiquidityCumulativeX128] = await observations.observeSingle(cache.blockTimestamp, 0n, slot0Start.tick, slot0Start.observationIndex, cache.liquidityStart, slot0Start.observationCardinality);

					cache.computedLatestObservation = true;
				}

				const liquidityNet = await Tick.cross(
					step.tickNext,
