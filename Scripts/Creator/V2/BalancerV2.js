...
function Data(callback_type) {
	...
	if (callback_type == 0) {
		...
		const callback = encoder.encode(["address", "address", "address", "bytes", "address", "uint256"], [VAULT.address, flashPool.token0_address, flashPool.token1_address, swapData, "0x0000000000000000000000000000000000000000", 0]);
...
function GetL2TransactionFee() {
	const getL2TransactionFee = `async function getL2TransactionFee() {
		...
		return (Number(transactionFeeL2) / 10 ** 18) * (await GlobalLibrary.getMarketPriceBatch(["ETH"]));
...
function Modules(chain, module_type, vault_address) {
	let modules;
...
function PoolObjects(pools) {
	...
	for (const pool of pools) {
		...
		const { pool_id, pool_address, pool_fee, token0_symbol, token1_symbol } = pool;

		const poolObjectName = `${token0_symbol}_${token1_symbol}_${pool_fee.replace(".", "_")}_${pool_id.slice(0, 6)}_POOL`;
...
async function Save(filePath, chain, module_type, vault_address, pools, callback_type, tx_type, native_token_symbol) {
	...
	const modules = Modules(chain, module_type, vault_address);
...
async function run() {
	for (const chain of chains) {
		...
		const { module_type, ProjectsV2, callback_type, tx_type, native_token_symbol } = require(`../../Common/Common/${chain}.js`);
		...
		for (const ProjectName in ProjectsV2) {
			...
			vault_address = ProjectsV2[ProjectName].factoryAddress[0];
		}
		...
		await Save(filePath, chain, module_type, vault_address, pools, callback_type, tx_type, native_token_symbol);
	}
}
