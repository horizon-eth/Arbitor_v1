...
function FlashPoolSelectors(pools) {
...
async function Save() {
	...
	const folderPath = "scripts/Dex/PoolDatasV2";
	...
	fs.readdir(folderPath, async (err, files) => {
		...
		for (const file of jsonFiles) {
			...
			const projectPath = `dapps/${projectName}Single.js`;
			...
			try {
				...
				const flashPoolSelectors = FlashPoolSelectors(pools);
