...
function GetL1TransactionFee(l1_fee_type, native_token_symbol) {
	...
	if (l1_fee_type == 0) {
		getL1TransactionFee = `async function calculateL1Fee() {
			...
			const L1GasFee = (((zeroBytes * 4 + nonZeroBytes * 16 + overhead) * L1FeeData.gasPrice * scalar) / 10 ** 27) * await GlobalLibrary.getMarketPriceBatch(["${native_token_symbol.toUpperCase()}"]);
...
async function Save(chain, module_type, callback_type, tx_type, l2_fee_type, l1_fee_type, native_token_symbol, part10_type) {
	...
	const folderPath = `Scripts/Dex/PoolDatasV2/${chain}`;
	...
	fs.readdir(folderPath, async (err, files) => {
		...
		for (const file of jsonFiles) {
			...
			if (projectName == "AntfarmV2") continue;
