...
function Initialize(native_token_symbol) {
	...
	async function initialize() {
		async function getTokenMarketPrices() {
			try {
				[token0_Market_Price, token1_Market_Price, nativeToken_Market_Price] = await GlobalLibrary.getMarketPriceBatch([token0_symbol, token1_symbol, "${native_token_symbol}"]);
			} catch (error) {
...
function GetL2TransactionFee(l2_fee_type, native_token_symbol) {
	...
	if (l2_fee_type == 0) {
		getL2TransactionFee = `async function getL2TransactionFee() {
			...
			return (Number(transactionFeeL2) / 10 ** 18) * (await GlobalLibrary.getMarketPriceBatch(["${native_token_symbol.toUpperCase()}"]));
		}
		...
		getL2TransactionFee = `async function getL2TransactionFee() {
			...
			const L2Fee = Number((gas.toString() * FeeData[chain].gasPrice) / 10 ** 18) * await GlobalLibrary.getMarketPriceBatch(["${native_token_symbol.toUpperCase()}"]);
		...
		getL2TransactionFee = `const transactionFeeL2 = Number((Number(gasLimit) * Number(FeeData[chain].gasPrice) * gasPrice_multiplier) / 10 ** 18) * nativeToken_Market_Price;`;
	} else if (l2_fee_type == -2) {
...
function GetL1TransactionFee(l1_fee_type, native_token_symbol) {
	...
	if (l1_fee_type == 0) {
		getL1TransactionFee = `async function calculateL1Fee() {
			...
			const L1GasFee = (((zeroBytes * 4 + nonZeroBytes * 16 + overhead) * L1FeeData.gasPrice * scalar) / 10 ** 27) * await GlobalLibray.getMarketPriceBatch(["${native_token_symbol.toUpperCase()}"]);
...
function Modules(chain, module_type) {
	...
	if (module_type == 0) {
		...
		modules = `
		const { ethers, ethersV5, fs, path, encoder, MIN_SQRT_RATIO, MAX_SQRT_RATIO, ERC20, flashSwapFunctionSelector, WalletAddress, flashSwap_gasLimit, gasLimit, overhead, scalar, FeeDataServerPort, maxFeePerGas_multiplier, maxPriorityFeePerGas_multiplier, gasPrice_multiplier  } = require("../../../Scripts/Common/Global/Global");
...
async function Save(filePath, chain, module_type, pools, callback_type, tx_type, l2_fee_type, l1_fee_type, native_token_symbol, part10_type) {
...
async function run() {
	for (const chain of chains) {
		...
		await Save(filePath, chain, module_type, pools, callback_type, tx_type, l2_fee_type, l1_fee_type, native_token_symbol, part10_type);
	}
}
